...
class SaleOrderLine(models.Model):
    ...
    pricing_hint = fields.Char(readonly=True, compute="_compute_pricing", store=False)
    ...
    def _onchange_sync_dates(self):
        for line in self:
            if line.product_id and line.product_id.must_have_dates:
                if line.start_datetime:
                    line.start_date = fields.Datetime.context_timestamp(
                        line, line.start_datetime
                    ).date()
                else:
                    line.start_date = False

                if line.end_datetime:
                    line.end_date = fields.Datetime.context_timestamp(
                        line, line.end_datetime
                    ).date()
                else:
                    line.end_date = False
